...
# Creates a log file and writes starting conditions
file = open("session.log", "a+")
file.write("{}\nTraining started: {}\n\n".format("-"*100 , datetime.now().strftime("%d/%m/%Y %H:%M:%S")))

file.write("Number of classes: {} | Number of epochs: {}\n".format(len(subjects), epochs))
file.write("Batch size: {} | Learning rate: {}\n".format(batch_size, learning_rate))
file.write("Training data size: {} | Test data size: {}\n\n".format(len(x_train), len(x_test)))
...
file.write("Epoch: 000 | Test accuracy: {0:0.3f}% | Train accuracy: {1:0.3f}%\n".format(test_acc, train_acc))
...
for epoch in range(epochs):
    ...
    file.write("Epoch: {0:03d} | Test accuracy: {1:0.3f}% | Train accuracy: {2:0.3f}% | Loss: {3:0.3f}\n".format(epoch+1, test_acc, train_acc, loss))
    
    loss_list.append(loss)
    test_list.append(test_acc)
    train_list.append(train_acc)

file.write("\nLoss: {}\nTest: {}\nTrain: {}\n\n".format(loss_list, test_list, train_list))
file.write("Training ended: {}\n{}\n".format(datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "-"*100))
file.close()
